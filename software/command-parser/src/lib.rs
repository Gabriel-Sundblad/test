...
/// use command_parser::*;
/// assert_eq!(
///     parse(b"duty 8 some text we don't care about"),
///     Some(Command::Duty(8))
/// );
/// ```
pub fn parse(bytes: &[u8]) -> Option<Command> {
    ...
    match next {
        ...
        b"duty" => {
            let next = split.next()?;
            let str = core::str::from_utf8(next).ok()?;
            let val: u8 = str.parse().ok()?;
            Some(Command::Duty(val))
        }
        _ => None,
    }
}
...
mod test_parse {
    ...
    #[test]
    fn test_parse5() {
        assert_eq!(parse(b"duty 8 some text that we don't care about"),
        Some(Command::Duty(8)))
    }

    #[test]
    fn test_parse6() {
        assert_eq!(parse(b"duty 800 some text that we don't care about"), None);
    }
...
pub fn parse_result(bytes: &[u8]) -> Result<Command, Error> {
    ...
    let result = match next {
        ...
        "duty" => {
            let next = split.next().ok_or(Error::ArgMissing)?;
            let val: u8 = next.parse().map_err(|_| Error::ArgError)?;
            Ok(Command::Duty(val))
        }
...
mod test_parse_result {
    ...
    #[test]
    fn test_parse_result_duty_error() {
        assert_eq!(
            parse_result(b"duty 800"),
            Err(Error::ArgError)
        );
    }
}
